...
async def main():
    ...
    try:
        chat = await client.get_input_entity('########')  # Replace with a known channel username
    ...
    try:
        chat = await client.get_input_entity('##########')  # Replace with the correct chat ID or username
        print(f"Successfully accessed chat: {chat}")
    except Exception as e:
        print(f"Error accessing chat: {e}")
    ...
    async def handle_bot1_message(event):
        ...
        if token_match:
            ...
            target_chat = '##############'  # Use the bot's username
            command = f'Your message: {token}'
    ...
    @client.on(events.NewMessage(chats='##########'))  # Use the bot's username
